...
void EditorLayer::OnUpdate(kx::Timestep ts)
{  
	kx_PROFILE_FUNCTION();
...
void EditorLayer::OnImGuiRender()
{
	...
	if (dockingEnabled)
	{
		...
		if (ImGui::BeginMenuBar())
		{
			if (ImGui::BeginMenu("File"))
			{
				...
				if (ImGui::MenuItem("Exit")) kx::Application::Get().Close();
				ImGui::EndMenu();
			}
		...
		auto stats = kx::Renderer2D::GetStats();
	...
	//	kx_CORE_INFO(m_ViewportFocused);
	//	kx_CORE_INFO( m_ViewportHovered);
		kx:: Application::Get().GetImGuiLayer()->BlockEvents(!m_ViewportFocused || !m_ViewportHovered);
...
void EditorLayer::OnEvent(kx::Event& e)
{
